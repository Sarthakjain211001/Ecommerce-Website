    ...
    const getProducts = async()=>{
      try{
        const res= await axios.get( 
          category 
          ? `http://localhost:5000/api/products/fetchAllProducts?category=${category}`    //If category is present then fetchdata using this link. When we will click on one of the cars from 3 cards present on the honepage. Then category will be passed in the url and this url will be used.
          : "http://localhost:5000/api/products/fetchAllProducts"                         // Else use this link. For displaying products on the home page this link will be used because no category will be passed and no category is required for data on home page
        );
